# ADR-050: Normalize product-vision ownership to docs/PRD.md
...
mixed in `docs/product-vision.md`. That mismatch caused conflicting guidance,
stale generated links, and ambiguous source selection when both files existed.

## Decision

Retire `docs/product-vision.md` as an active requirements artifact.
...
`docs/product-vision.md` is now a legacy filename only:
...
- **Only `docs/product-vision.md` (+/- features):** fail closed with migration
  guidance to move/copy the canonical product vision to `docs/PRD.md`; preserve
