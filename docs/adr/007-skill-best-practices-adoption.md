...
### 7. `.agent.md` Extension for Bootstrapped Agents

Agent files generated by `forge-build-agent-team` and copied by bootstrap scripts now use a `.agent.md` extension instead of plain `.md`. This allows harnesses to distinguish agent files from other markdown files using a simple glob pattern (`.agents/agents/*.agent.md`). The bootstrap scripts rename `.md` → `.agent.md` during copy. Backward compatibility is maintained — `forge-assign-models` and related skills fall back to `*.md` if no `.agent.md` files are found.

### 8. Extract `forge-orchestrate-build` Skill

Following the agent/skill separation pattern from ADR-001, the `project-orchestrator` agent was slimmed from ~710 to ~120 lines. All detailed procedural content (analysis steps, phase execution, output formatting, error handling, orchestration patterns, progress tracking) moved to a new `forge-orchestrate-build` skill at `templates/skills/forge-orchestrate-build/SKILL.md` with output format templates in `references/output-templates.md`.

The orchestrator agent now delegates to this skill, mirroring how `forge-team-builder` delegates to `forge-build-agent-team`.
...
- **Leaner agents.** The orchestrator agent is 83% smaller. Generated agent files distinguish themselves via `.agent.md` extension.
- **Consistent architecture.** Every forge agent now follows the same thin-shell + skill pattern.
- **Backward compatible.** Fallback `*.md` support ensures existing generated agents continue to work.

### Negative

- **Bootstrap script complexity.** Bootstrap scripts now rename files during copy (`.md` → `.agent.md`), adding ~2 lines of logic per script.
...
- **Bootstrap scripts updated.** Scripts now rename `.md` → `.agent.md` during copy and accept `--harness` presets (documented in ADR-006).
