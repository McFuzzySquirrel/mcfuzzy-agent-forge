...
Amended 2026-09-07: the first version of this ADR defaulted Claude-harness
repositories to the `claude` runner. That hid OpenCode's multi-provider
inventory from the Console, which has no runner selector, so the default was
restored and the selector is tracked as a follow-up.
