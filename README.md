...
**Latest: v3.27** - the workflow engine can now be **stopped gracefully**: `workflow-engine stop` (and a fixed `pause`) writes `docs/engine-control.json` and SIGTERMs the PID in `docs/engine.pid`, so a detached run finishes the current task, saves state as `paused`, and resumes with `run`. The same control is available from the launcher with `forge-launcher engine-run --stop` / `--pause`, and `forge-launcher resume` gains a "Stop the engine" option. Engine options (concurrency, keep-alive, retries, viz) persist to `docs/engine-config.json` so resume reuses them. See [docs/updates.md](docs/updates.md). (v3.26: adaptive keep-alive default + execution-budget hints. v3.25: launcher adds "stop here and resume later" checkpoints and a post-team plan & validate step. v3.24: the Copilot harness selects forge agents natively via `/agent <name>`. v3.23: the workflow engine's **output verification gate** stops hollow "complete" runs. v3.22: `forge-launcher` is the single terminal entry point with `resume`.)
