...
**Latest: v3.23** - the workflow engine's new **output verification gate** stops hollow "complete" runs: a task declaring `expectedOutputs` must actually produce them, and a task that changed nothing and only replied "Ready for the task." is retried then failed instead of being reported complete (`--allow-noop` relaxes it; `--run-validation` executes the manifest's validationCommands). The final summary and `status` flag tasks completed with no output files. See [docs/updates.md](docs/updates.md). (v3.22: `forge-launcher` is the single terminal entry point with `forge-launcher resume`, OSC 8 review links, and a conditional in-harness command; `forge-auto-build` is the terminal/headless fast-path. v3.21: the opencode harness selects the forge agent natively via `opencode run --agent`. v3.20: auto-drafted agent teams can no longer break the build with unquoted YAML frontmatter.)
