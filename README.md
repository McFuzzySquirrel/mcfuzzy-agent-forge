...
**Latest: v3.22** - `forge-launcher` is the single terminal entry point: new `forge-launcher resume [--repo]` re-enters any project at its current stage (idea → PRD → team → build) as an interactive wizard, review boundaries print OSC 8 terminal hyperlinks, and when a team already exists the launcher queues `/forge-orchestrate-build` (project-orchestrator) instead of `/forge-auto-build`. `forge-auto-build` is demoted to the terminal/headless fast-path; inside a harness you use `@project-orchestrator` (interactive) or `@workflow-orchestrator` (autonomous). See [docs/updates.md](docs/updates.md). (v3.21: the opencode harness selects the forge agent natively via `opencode run --agent`. v3.20: auto-drafted agent teams can no longer break the build with unquoted YAML frontmatter. v3.19: headless PRD runs get the same gap check the manual flow does. v3.18: the workflow engine runs multi-task opencode builds against one warm `opencode serve`.)
