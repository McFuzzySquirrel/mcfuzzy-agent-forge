...
| Signal | Mode |
|---|---|
| `docs/product-vision.md` exists, no `.agent.md` files in `HARNESS_AGENTS_DIR` | Greenfield |
| `HARNESS_AGENTS_DIR` contains `.agent.md` files (beyond forge templates) | Post-project |
...
2. Review existing agent files in `HARNESS_AGENTS_DIR` — load `forge-bootstrap-project/references/detect-harness.md` to determine this path — (`.agent.md` files) — domains, responsibilities, collaboration patterns.
