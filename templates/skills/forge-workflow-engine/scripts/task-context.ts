...
export function taskReferenceContext(repoRoot: string, task: ManifestTask): string {
  validateTaskContract(task);
  if (!task.contract) return "";
  const blocks = task.contract.references.map((path) => `### Reference: ${path}\n${readTaskFile(repoRoot, path)}`);
