...
Load this file when: the source document is a Feature PRD (has "Feature Overview", "Context: Existing System State", "Agent Impact Assessment") and existing `.agent.md` files already exist in `HARNESS_AGENTS_DIR`.
...
- **For new agents:** Write complete agent files at `HARNESS_AGENTS_DIR/{agent-name}.agent.md` following the standard template from Step 5.
