...
export function validateTaskContract(task: ManifestTask): void {
  ...
  strings(contract.references, "references", true).forEach(taskPath);
  ...
  if (contract.kind === "implementation" && task.expectedOutputs.some((path) => contract.references.includes(path))) throw new Error(`Task '${task.id}' must distinguish reference inputs from outputs.`);
  if (contract.kind === "human-review") {
    ...
    if (contract.references.includes(contract.reviewFile)) throw new Error("Human review file cannot overwrite a reference input.");
